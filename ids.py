...
def block_packet(src_ip, dest_ip, proto, src_port, dest_port):
    command = f"netsh advfirewall firewall add rule name='Block {src_ip} to {dest_ip}' dir=in action=block enable=yes profile=any localip={dest_ip} localport={dest_port} remoteip={src_ip} remoteport={src_port} protocol={proto}"
    try:
        subprocess.run(command, shell=True, check=True)
        logging.info(f"Blocked Packet: Source IP: {src_ip}, Destination IP: {dest_ip}, Protocol: {proto}, Source Port: {src_port}, Destination Port: {dest_port}")
    ...
    if 'IP' in pkt:
        try:
            ...
            for i in range(len(alertprotocols)):
                ...
                if flagpacket == True:
                    block_packet(src, dest, proto, sport, dport)
...
sniffthread = threading.Thread(target=scp.sniff, kwargs={"prn":pkt_process, "filter": "", "iface":[wifi_interface]}, daemon=True)
sniffthread.start()
